...
public class LiftRideWriter implements Runnable {
     ...
     */
    public LiftRideWriter(BlockingQueue<LiftRideWithIds> buffer, AtomicInteger successes, AtomicInteger failures) {
        this.buffer = buffer;
     ...
     */
    public LiftRideWriter(BlockingQueue<LiftRideWithIds> buffer, CountDownLatch latch, AtomicInteger successes, AtomicInteger failures) {
        this.buffer = buffer;
        this.latch = latch;
        this.successes = successes;
        this.failures = failures;
    ...
    private void apiSetup(SkiersApi apiInstance) {
        String BASE_PATH = "http://54.244.161.83:8080/server_war";
    ...
    /**
     * Actual implementation of the post with retry method. Recursively calls itself until max # of tries.
     */
    private void postWithRetry(LiftRide liftRide, LiftRideWithIds liftRideWithIds, int trialsMade, int maxTries) {
        ...
        try {
            ...
            ApiResponse<Void> response = apiInstance.writeNewLiftRideWithHttpInfo(liftRide, liftRideWithIds.getResortId(),
                    liftRideWithIds.getSeasonId(), liftRideWithIds.getDayId(), liftRideWithIds.getLiftId());
            // Timestamp 2
            // Timestamp Analytics
